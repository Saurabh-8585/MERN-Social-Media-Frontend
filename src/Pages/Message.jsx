...
const Message = () => {
  ...
  const combinedUsers = (userFollowers && userFollowings) &&
    Array.from(
      new Set([...userFollowings, ...userFollowers].map(user => user.id))
    )
      .map(userId => {
        const followingUser = userFollowings.find(user => user.id === userId);
        const followerUser = userFollowers.find(user => user.id === userId);
  ...
  const messages = [
    ...
    { id: 2, sender: 'Jane Smith', content: 'Hi! How are you?' },
    { id: 3, sender: "John Doe', content: 'I'm good. How about you?" },
    { id: 4, sender: 'Robert Johnson', content: 'Hey! What are you up to?' },
  ];
  console.log({ combinedUsers });
  return (
    <div className="flex justify-center items-center flex-col mt-10 p-5 w-full max-w-xl m-auto">
      <div class="container mx-auto justify-center w-full max-w-xl">
        <div class="w-full border rounded lg:grid lg:grid-cols-3 max-w-xl">
          <div class="border-r border-gray-300 lg:col-span-1 w-full max-w-xl">
            <div class="mx-3 my-3">
